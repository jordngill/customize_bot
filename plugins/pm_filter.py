...
async def left_chat(client, message):
    await remove_group(client.databasee, message.chat.id)
...
async def api(client, message):
    if message.chat.id not in ADMINS:
        ...
        if args:
            ...
            if res:
                return await message.reply('Api key setup **successful**')

        reply_markup = InlineKeyboardMarkup([
            [InlineKeyboardButton('HOW TO SET API KEY ?', callback_data='how_to_set_api_key')]
        ])
        
        await message.reply(
            'Failed to set **api key**. Click on the below button to learn more.',
            reply_markup=reply_markup
        )
    else:
        await message.reply('This feature is not for admins')
...
async def next_page(bot, query):
    ...
    if query.chat.type in (ChatType.GROUP, ChatType.SUPERGROUP):
        down_user = await get_down_user(bot.database, query.chat.id)

        if down_user:
            user_cred = await get_user_creds(bot.database, down_user)

            if user_cred:
                end_point, api_key = user_cred
        ...
        btn = [
            [
                InlineKeyboardButton(
                    ...
                    url=await get_shortlink(f"https://telegram.dog/{temp.U_NAME}?start=files_{file.file_id}", end_point, api_key)
                ),
            ]
            for file in files
        ]
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('⚡ SUBSCᏒIBΞ ⚡', url='https://youtube.com/@gillharyana'),
            InlineKeyboardButton('🤖 UᎮDΛTΞS 🤖', url='https://t.me/Ecommerce_Shop')
        ], [
        ...
        return await query.message.edit_text(
            text=f'Hi **{query.from_user.first_name}**, You can also starting earning using this bot. Just set your **shortener** and **api key**. And **add** this bot in your **group**, Your users can **search** for **movie** and you will **earn** for every single user who watches using your link. To know more checkout the below options.',
            reply_markup=reply_markup
        )

    elif query.data == "how_to_set_shortener":
        ...
        reply_text = (
            'Which ever **shortener** you want to use, Go to there '
            '**website > tools > developers api**. Then copy the api url (not your api key) '
            'and replace the value of **api key** and **destination url** fields with **{}**. '
            'Then send **/shortener api_url**'
        )
    ...
    elif query.data == "how_to_set_api_key":
        ...
        reply_text = (
            'Which ever **shortener** you are using, Go to there '
            '**website > tools > developers api**. Then copy your api key '
            'and **send** like this **/api api_key**'
        )
